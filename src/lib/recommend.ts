...
const BLOCK_HOURS: Record<BlockId, number> = {
  pagi: 4,
  siang: 4,
  sore: 4,
  malam: 4,
...
function buildFor(
  ...
  const flagged = (c: HexCell, b: BlockId) =>
    tier === "prioritas"
      ? c.blocks[b].gap === kind
      : c.blocks[b].watch &&
        (c.nearestTransitM > WALK_M ? "jangkauan" : "jadwal") === kind;
  const pool =
    tier === "prioritas"
      ? model.cells
      : model.cells.filter((c) => !TIME_BLOCKS.some((b) => c.blocks[b.id].gap)); // sudah prioritas, jangan dihitung dua kali
  const gapCells = pool.filter((c) =>
    TIME_BLOCKS.some((b) => flagged(c, b.id)),
  );
  ...
  return clusters(gapCells).map((cl) => {
    ...
    for (const b of TIME_BLOCKS) {
      const pts = cl.reduce(
        (s, c) => s + (flagged(c, b.id) ? c.blocks[b.id].total : 0),
        0,
      );
      if (pts > domPts) {
        domPts = pts;
    ...
    const avgNodeM = Math.round(
      cl.reduce((s, c) => s + c.nearestNodeDistM, 0) / cl.length,
    );
    const avgTransitM = Math.round(
      cl.reduce((s, c) => s + c.nearestTransitM, 0) / cl.length,
    );
    const service =
      cl.reduce((s, c) => s + c.blocks[domBlock].service, 0) / cl.length;
    const railDep = Math.round(
      cl.reduce((s, c) => s + c.blocks[domBlock].railDep, 0) / cl.length,
    );
    const busDep = Math.round(
      cl.reduce((s, c) => s + c.blocks[domBlock].busDep, 0) / cl.length,
    );
    ...
    const levelWord =
      tier === "prioritas" ? "ramai" : "cukup ramai (tingkat sedang)";
    ...
    if (kind === "jangkauan") {
      ...
      const from = useStop
        ? { lat: stop!.lat, lon: stop!.lon, name: `Halte ${stop!.name}` }
        : { lat: nearest.lat, lon: nearest.lon, name: nearest.name };
      ...
      const proposal: Proposal = {
        ...
        points: [
          ...
          {
            label: "Panjang",
            value: `sekitar ${km(lengthM)} km, berhenti di dalam kawasan`,
          },
          {
            label: "Uji coba",
            value: `${fleet} unit, tiap ${headway} menit, blok ${blok}`,
          },
        ],
        steps: [
          tier === "prioritas"
        ...
        caveat:
          "Angka kasar untuk memulai kajian, bukan rencana operasi. Kapasitas, tarif, dan trayek final ditentukan operator dan Dishub.",
      };
      ...
      return {
        ...
        rankParts: [
          `${cl.length} petak `,
          `${evidence} laporan warga`,
          "Tak terjangkau diberi bobot lebih besar",
          `Skor ${score}`,
        ],
      };
    }

    // Moda yang diusulkan = moda yang benar-benar menjangkau kawasan ini (skor tertinggi),
    // bukan yang keberangkatannya paling banyak: stasiun jauh dengan 90 kereta bukan solusi.
    const railScore =
      cl.reduce(
        (s, c) =>
          s +
          distanceFactor(c.nearestNodeDistM) *
            Math.min(
              1,
              c.blocks[domBlock].railDep / model.refDep.rail[domBlock],
            ),
        0,
      ) / cl.length;
    const busScore =
      cl.reduce(
        (s, c) =>
          s +
          distanceFactor(c.nearestStopDistM) *
            Math.min(1, c.blocks[domBlock].busDep / model.refDep.bus[domBlock]),
        0,
      ) / cl.length;
    const railTarget = railScore >= busScore;
    const hours = BLOCK_HOURS[domBlock];
    const ref = railTarget
      ? model.refDep.rail[domBlock]
      : model.refDep.bus[domBlock];
    const nowDep = railTarget ? railDep : busDep;
    const targetDep = Math.max(nowDep + 1, Math.ceil(TARGET_SERVICE * ref));
    const extra = targetDep - nowDep;
    ...
    const fleet = Math.max(
      1,
      Math.ceil(((extra / 2) * cycleMin) / (hours * 60)),
    );
    ...
    const proposal: Proposal = {
      ...
      points: [
        ...
        {
          label: "Keberangkatan",
          value: `sekitar ${nowDep} menjadi ${targetDep} per blok`,
        },
        {
          label: "Jeda",
      ...
      caveat:
        "Angka kasar untuk membuka pembicaraan dengan operator. Ketersediaan rangkaian, slot jalur, dan biaya tidak dihitung SIMPUL.",
    };
    ...
    return {
      ...
      target: railTarget
        ? "KAI Commuter / operator rel"
        : "TransJakarta / JakLingko",
      targetShort: railTarget
        ? ("KAI Commuter" as const)
        : ("TransJakarta" as const),
      ...
      rankParts: [
        `${cl.length} petak`,
        `${evidence} laporan warga`,
        `Skor layanan ${Math.round(service * 100)} dari 100 (makin tipis, makin tinggi urutannya)`,
...
export function buildRecommendations(model: SimpulModel): Recommendation[] {
  ...
  const prioritas = [
    ...buildFor(model, "jangkauan", "prioritas").sort(byScore).slice(0, 8),
    ...buildFor(model, "jadwal", "prioritas").sort(byScore).slice(0, 8),
  ]
    .sort(byScore)
    .slice(0, 12);
  // Pantau: paling banyak 10, nomornya melanjutkan nomor prioritas.
  const pantau = [
    ...buildFor(model, "jangkauan", "pantau"),
    ...buildFor(model, "jadwal", "pantau"),
  ]
    .sort(byScore)
    .slice(0, 10);
